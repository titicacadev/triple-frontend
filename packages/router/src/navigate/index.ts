...
import { checkIfRoutable, generateUrl, parseUrl } from '@titicaca/view-utilities'
...
function useNavigate({
  ...
  const navigateInApp = useCallback(
    (rawHref: string, { onFailRoutableInApp, onFailRoutableInBrowser: _, ...params }: NavigateOptions) => {
...
export function useAuthNavigate({
  ...
  return { 
    navigate: (rawHref: string, params?: OutlinkOptions) => _navigate(rawHref, {
      ...params,
      onFailRoutableInApp: showLoginCtaModal,
      onFailRoutableInBrowser: () => showTransitionModal(TransitionType.General)

    }),
    openWindow: (rawHref: string, params?: OutlinkParams) => _openWindow(rawHref, {
      ...params,
